...
public class SimplePanel extends JPanel{
	...
	public void paintComponent(Graphics g) {
		...
		for(Circle c:content.getCircles()){
			...
			Ellipse2D.Double shape = new Ellipse2D.Double(c.getTopLeftPoint().getX()-(c.getDiameter()/2) ,c.getTopLeftPoint().getY()-(c.getDiameter()/2), c.getDiameter(), c.getDiameter() );
		    g2d.draw(shape);
		...
		for(Label l: content.getLabels()){
			...
			g2d.drawString(l.getText(), Math.round(l.getPosition().getX())-12 , Math.round(l.getPosition().getY())-3 );
		}
